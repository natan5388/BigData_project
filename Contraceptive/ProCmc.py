...
#Data Source: https://archive.ics.uci.edu/ml/datasets/Contraceptive+Method+Choice
...
dataf = pd.read_csv('Contraceptive/cmc.csv')
...
idade = dataf["wAge"] 
nfilhos = dataf["nFilhos"]
#o resto dos valores são categoricos como diz no repositorio
...
print("Imprimindo sobre a idade da esposa...")
menor = dataf[(dataf["wAge"] < dataf["wAge"].mean())]
print("Menor que a media:\n", menor)
maior = dataf[(dataf["wAge"] > dataf["wAge"].mean())]
print("Maior que a media:\n", maior)
#
print("Imprimindo sobre o numero de filhos...") 
menor = dataf[(dataf["nFilhos"] < dataf["nFilhos"].mean())]
print("Menor que a media:\n", menor)
maior = dataf[(dataf["nFilhos"] > dataf["nFilhos"].mean())]
print("Maior que a media:\n", maior)
#
"""
FAZENDO PLOT COM O DF
"""
classes = np.unique(dataf['metodoContra'])
atrib1 = 0  #idade
atrib2 = 3  #numero de filhos
...
for i in range(len(classes)):
    ...
    classe = dataf.loc[dataf['metodoContra']==classes[i]]
...
plt.legend(["Não usa", "Curto", "Longo"])
plt.show()
